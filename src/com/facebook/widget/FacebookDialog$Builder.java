...
public abstract class FacebookDialog$Builder<CONCRETE extends Builder<?>>
{
  ...
  {
    imageAttachmentFiles.put(paramString, paramFile);
    return this;
  }
  ...
  public FacebookDialog build()
  {
    ...
    for (Bundle localBundle = getMethodArguments();; localBundle = setBundleExtras(new Bundle()))
    {
      ...
      FacebookDialog.access$500(activity, fragment, FacebookDialog.access$400(str, localBundle.containsKey("com.facebook.platform.extra.PHOTOS")), "Failed");
    ...
    new FacebookDialog.OnPresentCallback()
    {
        ...
        throws Exception
      {
        if ((imageAttachments != null) && (imageAttachments.size() > 0)) {
          FacebookDialog.access$900().addAttachmentsForCall(paramAnonymousContext, appCall.getCallId(), imageAttachments);
        }
        if ((imageAttachmentFiles != null) && (imageAttachmentFiles.size() > 0)) {
          FacebookDialog.access$900().addAttachmentFilesForCall(paramAnonymousContext, appCall.getCallId(), imageAttachmentFiles);
        }
      }
    };
  }
