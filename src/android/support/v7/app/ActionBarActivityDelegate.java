...
abstract class ActionBarActivityDelegate
{
  ...
  {
    paramBundle = mActivity.obtainStyledAttributes(R.styleable.ActionBarWindow);
    if (!paramBundle.hasValue(0))
    {
      paramBundle.recycle();
      throw new IllegalStateException("You need to use a Theme.AppCompat theme (or descendant) with this activity.");
    }
    mHasActionBar = paramBundle.getBoolean(0, false);
    mOverlayActionBar = paramBundle.getBoolean(1, false);
    paramBundle.recycle();
    ...
    implements ActionBarDrawerToggle.Delegate
  {
    ...
    {
      TypedArray localTypedArray = mActivity.obtainStyledAttributes(new int[] { getHomeAsUpIndicatorAttrId() });
